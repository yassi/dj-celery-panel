...
test_install: build
	python -m pip uninstall -y $(PACKAGE_NAME) || true
	python -m pip install -e .
	python -c "import dj_celery_panel; print('✅ Import success!')"
...
test_coverage:
	...
	@docker compose exec dev bash -c "cd /app && REDIS_HOST=redis python -m pytest --cov=dj_celery_panel --cov-report=xml --cov-report=html --cov-report=term-missing tests/"
...
docker_up:
	...
	@echo "   Dev container: dj-celery-panel-dev-1"
	@echo ""
